...
from Common.util.log_1 import Logger
log_name = "sendmail.log"
log = Logger(log_name).get_logger()


class Email:
    def __init__(self, server, sender, password, receiver, title, message=None, path=None,logger=log):
        """初始化Email
    ...
    def send(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.info('发送邮件"{0}"成功! 收件人：{1}。如果没有收到邮件，请检查垃圾箱，同时检查收件人地址是否正确'.format(self.title, self.receiver))