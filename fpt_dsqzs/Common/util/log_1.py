...
from Common.util.edit_file.edit_ymal import Config
from Common.util.get_dir import *


# 定义全局参数
log_file = os.path.join(get_how_dir(os.path.abspath(__file__), 3), "log")   # 定义日志文件存放目录


class Logger(object):
    def __init__(self, log_name,logger_name='framework'):
        self.logger = logging.getLogger(logger_name)
        logging.root.setLevel(logging.NOTSET)
        c = Config("ceshihuanjing_config.yml").get('log')
        self.log_file_name = log_name   # 日志文件
    ...
    def get_logger(self):
        ...
        if not self.logger.handlers:  # 避免重复日志
            ...
            file_handler = TimedRotatingFileHandler(filename=os.path.join(log_file, self.log_file_name),
                                                    when='D',
                                                    interval=1,
